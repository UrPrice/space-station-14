...
public abstract class SharedAbsorbentSystem : EntitySystem
{
    ...
    // I can fck it up in upstream
    public bool Mop(Entity<AbsorbentComponent> absorbEnt, EntityUid user, EntityUid target) // SS220 Placeable-Items
    {
        ...
        if (TryPuddleInteract((absorbEnt.Owner, absorbEnt.Comp, useDelay), absorberSoln.Value, user, target)
            ...
            return true; //SS220-Placeable-item

        // If it's refillable try to transfer
        return TryRefillableInteract((absorbEnt.Owner, absorbEnt.Comp, useDelay), absorberSoln.Value, user, target); //SS220-Placeable-item
    }

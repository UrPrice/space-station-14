...
public abstract class SharedSuitSensorSystem : EntitySystem
{
    ...
    public bool TrySetSensor(Entity<SuitSensorComponent?> sensors, SuitSensorMode mode, EntityUid userUid)
    {
        ...
        // ss220 tweak sensors start
        if (sensors.Comp.User != null && userUid != sensors.Comp.User && !_actionBlocker.CanInteract(sensors.Comp.User.Value, null))
        {
        ...
        else
        {
            ...
            // ss220 tweak sensors start
            var doAfterArgs = new DoAfterArgs(EntityManager, userUid, sensors.Comp.SensorsTime, doAfterEvent, sensors, target: sensors.Comp.User)
            {
                BreakOnMove = true,
                BreakOnDamage = true
            };
            ...
            if (_doAfterSystem.TryStartDoAfter(doAfterArgs))
                _popupSystem.PopupEntity(Loc.GetString("suit-sensor-do-after-start"), sensors.Comp.User.Value, sensors.Comp.User.Value);
            // ss220 tweak sensors end
        }
        return true;
    }

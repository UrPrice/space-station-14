...
{
    [Dependency] private readonly IGameTiming _gameTiming = default!; // SS220-add-emote-cooldown;
    ...
    protected bool TryEmoteChatInput(EntityUid source, string textInput)
    {
        ...
        if (TryComp<EmotingComponent>(source, out var comp))
        {
    ...
    public bool AllowedToUseEmote(EntityUid source, EmotePrototype emote)
    {
        ...
        if (TryComp<SpeechComponent>(source, out var speech) &&
            (speech.AllowedEmotes.Contains(emote.ID) || speech.ClothingEmotes.Contains(emote.ID))) //SS220 Clothing special emotes
        {
            return true;
        }

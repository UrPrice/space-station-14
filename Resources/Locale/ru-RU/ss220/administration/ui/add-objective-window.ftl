ui-add-objective-title = Добавить цель

ui-add-custom-objective-text = Добавление свободной цели
ui-add-usual-objective-text = Добавление прототипной цели

ui-add-custom-objective-name-placeholder = Цель
ui-add-custom-objective-desc-placeholder = Описание цели
ui-add-custom-objective-icon-placeholder = Путь к иконке (необязательно)
ui-add-custom-objective-issuer-placeholder = Принадлежность к организации (необязательно)

ui-add-objective-option-button-placeholder = Прототип не выбран
ui-add-objective-entity-id-line-edit-placeholder = Введите ID сущности цели или прототип
ui-add-objective-apply = Добавить
ui-add-objective-force-assign-text = Принудительное добавление

ui-add-objective-chat-manager-announce = Ваш список целей был обновлен! Ознакомиться с ними можно через меню персонажа.

ui-toggle-status-objective-button = Сменить статус цели

...
public sealed partial class AdminLogsControl : Control
{
    ...
    public AdminLogsControl()
    {
        ...
        // this need to correctly set color modulation
        ...
        EarlyBorderTime.Text = DateTime.UtcNow.ToString();
        LateBorderTime.OnTextEntered += OnLateDateTimeChanged;
        LateBorderTime.OnTextChanged += (_) => LateBorderTime.ModulateSelfOverride = null;
        LateBorderTime.Text = DateTime.UtcNow.ToString();
        CopyToClipboard.OnButtonDown += (_) => OnCopyToClipboard();

        ClearSelection.SetClickState(1, new ConfirmableButtonState(Loc.GetString("admin-logs-clear-selection-confirm"), StyleNano.ButtonColorCautionDefault));
        ClearSelection.OnConfirmed += () =>
        {
            foreach (var child in LogsContainer.Children)
            {
                ...
                log.MarkedForCopying = false;
            }
        };

        // this is disaster...
        SelectorModeIndicator.SetFromSpriteSpecifier(new SpriteSpecifier.Texture(_selectorIndicatorTexture));
        SelectorModeIndicator.DisplayRect.TextureScale = new(0.1f); // 192 -> 32 and some smallerfy
        // SS220-add-logs-copying-end
    ...
    private void UpdateLogs()
    {
        ...
        foreach (var child in LogsContainer.Children.Reverse()) // SS220-make-earlier-logs-be-up
        {
            if (child is not AdminLogLabel log)
            {
                continue;
    ...
    private bool ShouldShowLog(AdminLogLabel label)
    {
        ...
        //SS220-add-time-checker-begin
        if (!PassEarlyTimeFilter(label.Log))
            return false;

        if (!PassLateTimeFilter(label.Log))
            return false;
        //SS220-add-time-checker-end

...
namespace Content.Client.PDA
{
    ...
    public sealed partial class PdaMenu : PdaWindow
    {
        ...
        // ss220 add additional info for pda start
        private static readonly ProtoId<FontPrototype> AccessFont = "Default";
        ...
        public PdaMenu()
        {
            ...
            // ss220 add additional info for pda start
            var resProto = _proto.Index(AccessFont);
            var fontProto = _resCache.GetResource<FontResource>(resProto.Path);
            _font = new VectorFont(fontProto, 10);
            // ss220 add additional info for pda end
        ...
        public void UpdateState(PdaUpdateState state)
        {
            ...
            // ss220 add additional info for pda start
            IdPhoto.ResetCharacterSpriteView();
            AvailableAccessTags.RemoveAllChildren();
            // ss220 add additional info for pda end
            ...
            if (state.PdaOwnerInfo.IdOwner != null || state.PdaOwnerInfo.JobTitle != null)
            {
                ...
                // ss220 add additional info for pda start
                IdPhotoContainer.Visible = false;

                if (state.PdaOwnerInfoExtended is { Record: not null, Record.Profile: not null })
                {
                    IdPhotoContainer.Visible = true;
                    IdPhoto.SetupCharacterSpriteView(state.PdaOwnerInfoExtended.Record.Profile, state.PdaOwnerInfoExtended.Record.JobPrototype);
                }

                AccessContainer.Visible = false;

                var idCard = _entMan.GetEntity(state.PdaOwnerInfoExtended.IdCard);
                if (idCard != null && _entMan.TryGetComponent<AccessComponent>(idCard.Value, out var access))
                {
                    ...
                    foreach (var tag in access.Tags)
                    {
                        ...
                        AvailableAccessTags.AddChild(accessLabel);
                    }

                    if (AvailableAccessTags.ChildCount == 0)
                        AccessContainer.Visible = false;
                }
                // ss220 add additional info for pda end
            }

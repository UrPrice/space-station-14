...
public sealed partial class RecordList : ScrollContainer
{
    ...
    private readonly Dictionary<string, DepartmentPrototype> _jobIdToDepartment = new();
    ...
    private void MapJobsToDepartments()
    {
        ...
        foreach (var department in departments)
        {
            ...
            {
                _jobIdToDepartment.TryAdd(roleId, department);
            }
        }
    }
    ...
    public string DicideGroup(CriminalRecordShort record)
    {
        ...
        if (record.LastCriminalRecord is { RecordType: not null })
            return CriminalGroupId;

        if (string.IsNullOrEmpty(record.JobPrototype) ||
            !_jobIdToDepartment.TryGetValue(record.JobPrototype, out var department) ||
            !RecordListGroups.ContainsKey(department.ID))

...
public sealed class NukeopsRuleSystem : GameRuleSystem<NukeopsRuleComponent>
{
    ...
    private void OnWarDeclared(ref WarDeclaredEvent ev)
    {
        ...
        Dictionary<NukeopsRuleComponent, LoneOpsSpawnRuleComponent> opsRuleDict = [];
        var loneopsQuery = EntityQueryEnumerator<LoneOpsSpawnRuleComponent>();
        while (loneopsQuery.MoveNext(out _, out var loneOpsRule))
        {
            if (!loneOpsRule.AdditionalRule.HasValue)
                continue;

            if (!TryComp<NukeopsRuleComponent>(loneOpsRule.AdditionalRule, out var nukeopsRule))
                continue;

            opsRuleDict.TryAdd(nukeopsRule, loneOpsRule);
        }
    ...
    private void DistributeExtraTc(NukeopsRuleComponent nukieRule, LoneOpsSpawnRuleComponent? loneOpsRule = null)
    {

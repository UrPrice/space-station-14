...
public sealed class CluwneCommsConsoleSystem : EntitySystem
{
    ...
    private void UpdateUI(EntityUid ent, CluwneCommsConsoleComponent comp)
    {
        ...
        CluwneCommsConsoleInterfaceState newState = new CluwneCommsConsoleInterfaceState(comp.CanAnnounce,
            comp.CanAlert,
            levels,
            comp.AnnouncementCooldownRemaining,
            comp.AlertCooldownRemaining);
    ...
    private void OnAnnounceMessage(Entity<CluwneCommsConsoleComponent> ent, ref CluwneCommsConsoleAnnounceMessage args)
    {
        ...
        _adminLogger.Add(LogType.Chat,
            LogImpact.Low,
            $"{ToPrettyString(args.Actor):player} has sent the following station announcement: {msg}");
    ...
    private void OnAlertMessage(Entity<CluwneCommsConsoleComponent> ent, ref CluwneCommsConsoleAlertMessage args)
    {
        ...
        _adminLogger.Add(LogType.Chat,
            LogImpact.Medium,
            $"{ToPrettyString(args.Actor):actor} created news article {article.Title} by {article.Author}: {article.Content}");
